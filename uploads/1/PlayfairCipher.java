...
for (int j = 0; j < 5; j++) 
{
matrix_arr[i][j] = Key.charAt(counter); System.out.print(matrix_arr[i][j] + " ");
counter++;
}
System.out.println();
}
...
private String[] Divide2Pairs(String new_string) 
{
int size = new_string.length();
if (size % 2 != 0) 
{
size++;
...
{
x[i] = new_string.substring(counter, counter + 2);
counter = counter + 2;
}
return x;
}
...
{
String src_arr[] = Divide2Pairs(format(Source)); String Code = new String();
char one;
char two;
int part1[] = new int[2];
int part2[] = new int[2];
...
x.setKey(keyword);
x.KeyGen();
System.out.println("Enter word to encrypt: (Make sure the length of the message is even)");
